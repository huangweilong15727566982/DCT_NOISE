...
parser.add_argument('--gen_epoch', default=30, type=int)
parser.add_argument('--use_subset', action='store_true', default=False)
parser.add_argument('--attack_type', default='min-min', type=str, choices=['min-min','min-min2', 'min-max','min-max2','dct','dct2','random'], help='Attack type')
...
parser.add_argument('--channel', default=None, type=float, help='perturb channel nums')
parser.add_argument('--random_start', action='store_true', default=False)
args = parser.parse_args()

# Convert Eps
if not args.attack_type.startswith('dct'):
...
pretrained_resnet="experiments/weights/checkpoints/resnet18"
...
def universal_perturbation_eval(noise_generator, random_noise, data_loader, model, eval_target=args.universal_train_target):
    ...
    for  (images, labels) in data_loader[eval_target]:
...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    while condition: #stop condition
        step=step+1
        if (args.attack_type.startswith('min-min') or args.attack_type.startswith('dct')) and not args.load_model:
            # Train Batch for min-min noise
            for j in tqdm(range(0, args.train_step)): 
                try:
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader[args.universal_train_target]), total=len(data_loader[args.universal_train_target])):
            ...
            elif args.attack_type =='dct':
                perturb_img, eta = noise_generator.dct_attack(images, labels, model, optimizer, criterion, random_noise=batch_noise)
            elif args.attack_type =='dct2':
                perturb_img, eta = noise_generator.dct_attack2(images, labels, model, optimizer, criterion, random_noise=batch_noise)
            #class targeted attack 
            elif args.attack_type == 'min-max2':
                perturb_img, eta = noise_generator.min_max_attack2(images, labels, model, optimizer, criterion, random_noise=batch_noise)
            else:
                raise('Invalid attack')

            class_noise_eta = collections.defaultdict(list)
            for i in range(len(eta)): #genxin
            ...
            for key in class_noise_eta:
                ...
                class_noise += delta
                if args.attack_type.startswith('dct'):
                    # batch_class_noise=class_noise.unsqueeze(0)
                    random_noise[key]=class_noise
                    #noise_generator.idct_on_batch(torch.clamp(noise_generator.dct_on_batch(batch_class_noise)[0], -args.epsilon, args.epsilon).unsqueeze(0))[0]
                    # print("nihao",args.epsilon)
                else:
        ...
        if args.attack_type.startswith('min-min') or args.attack_type.startswith('dct'):
            condition = error_rate > args.universal_stop_error
        if step>=args.gen_epoch: 
            condition=False
    return random_noise
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    while condition:
        step+=1
        if (args.attack_type.startswith('min-min') or args.attack_type.startswith('dct')) and not args.load_model:
            # Train Batch for min-min noise or dct noise
        ...
        # if args.attack_type.startswith('min-min') or args.attack_type.startswith('dct'):
        #     condition = error_rate > args.universal_stop_error
        # elif args.attack_type.startswith('min-max'):
            #condition = error_rate < args.universal_stop_error
        if step>=args.gen_epoch: 
            condition=False
...
def main():
    ...
    model=mlconfig.getcls(config.model)(num_classes=config.model.num_classes).to(device)
    ...
    scheduler = mlconfig.getcls(config.scheduler)(optimizer,T_max=config['scheduler']['T_max'])
    criterion = mlconfig.getcls(config.criterion)()
    ...
    if args.attack_type.startswith('dct'):
        noise_generator = toolbox.PerturbationTool(epsilon=args.epsilon,
                                               ...
                                               step_size=args.step_size,c=args.channel)
    else:
        noise_generator = toolbox.PerturbationTool(epsilon=args.epsilon,
                                               num_steps=args.num_steps,
    ...
    elif args.attack_type.startswith('min-min') or args.attack_type.startswith('min-max') or args.attack_type.startswith('dct'):
        if args.attack_type.startswith('min-max') and not args.load_model:
            checkpoint = util.load_model(filename=pretrained_resnet,
                                     model=model,
                                     optimizer=optimizer,
                                     alpha_optimizer=None,
                                     scheduler=scheduler)
